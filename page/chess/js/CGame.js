                ...
                case BOARD_SPECIAL_CASTLING_RIGHT:
                    {
                        ...
                        var oDestinationCell = _aCell[_oCellActive.getLogicPos().row][5];oRightRook.shift(oDestinationCell);
                        break;
                    }
                case BOARD_SPECIAL_CASTLING_LEFT:
                    {
                        var oLeftRook = _aCell[_oCellActive.getLogicPos().row][0];
                        var oDestinationCell = _aCell[_oCellActive.getLogicPos().row][3];oLeftRook.shift(oDestinationCell);
                        break;
                    }
                case BOARD_SPECIAL_ENPASSANT:
                    {
                        var oPawnToPass = _aCell[_oCellActive.getLogicPos().row][iCol];oPawnToPass.eatUp();oPawnToPass.setPiece(null);
                        break;
                    }
            }
    ...
    this.checkGameState = function() {
        ...
        switch (iCheckState) {
            case BOARD_STATE_STALEMATE:
                {
                    this.gameOver(DRAW);_oMessage = new CMessage(_iCurPlayer, TEXT_STALEMATE, _oInfoPanelContainer);bEndGame = true;
                    break;
                }
            ...
            case BOARD_STATE_CHECKMATE:
                {
                    var iWinner = s_oBoardStateController.getOtherOpponent(_iCurPlayer);_oMessage = new CMessage(_iCurPlayer, TEXT_CHECKMATE, _oInfoPanelContainer);this.gameOver(iWinner);bEndGame = true;
                    break;
                }
        }
        return bEndGame;
